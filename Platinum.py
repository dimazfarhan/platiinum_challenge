...
#Neural Network
pickle.load(open("model_of_neural/model-sentiment.p", "rb"))
pickle.load(open("model_of_neural/feature.p", "rb"))
...
@swag_from("docs/neural_text.yml", methods=['POST'])
@app.route('/neural_text', methods=['POST'])
def neural_text():
    ...
    count_vect = pickle.load(open("model_of_neutral/feature.p", "rb"))

    # Ubah text menjadi vektor menggunakan CountVectorizer yang sama
    text_vect = count_vect.transform(text_list)

    model = pickle.load(open("model_of_neutral/model-sentiment.p", "rb"))
...
@swag_from("docs/neural_file.yml", methods=['POST'])
@app.route('/neural_file', methods=['POST'])
def nn_file():
    if 'upload_file' not in request.files:
        return redirect(request.url)
